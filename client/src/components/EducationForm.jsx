                    ...
                    {data.map((education,index) => {
                        ...
                        <div key={index} className="p-4 border border-gray-200 rounded-lg space-y-3">
                            ...
                            <div className="grid md:grid-cols-2 gap-3">
                                ...
                                <input value={education.degree || ""} onChange={(e) => updateEducation(index,"degree",e.target.value)} type="text" placeholder="Degree (eg. Bachelor's, Master's)" className="px-3 py-2 text-sm "/> 
                                <input value={education.field || ""} onChange={(e) => updateEducation(index,"field",e.target.value)} placeholder="Field of study" type="text" className="px-3 py-2 text-sm "/> 
                                <input value={education.graduation_date || ""} onChange={(e) => updateEducation(index,"graduation_date",e.target.value)} type="month" 
                                className="px-3 py-2 text-sm"/>
                            </div>
                            
                            <input value={education.gpa || ""} onChange={(e) => updateEducation(index,"gpa",e.target.value)} placeholder="GPA (optional)" type="text" className="px-3 py-2 text-sm "/> 
                        </div>
                    )})}
                </div>
            )}
